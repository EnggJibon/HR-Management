...
namespace SecurityAdministration.DAL.Repositories
{
    ...
    {
        public RoleWiseScreenPermissionRepository(ERP_Security context)
            : base(context)
        {
        }
        ...
        public IEnumerable<ScreenPermissionView> Get(int? roleID = null, string moduleID = "", string screenCode = "")
        {
            ...
            query.Append(" CASE WHEN SUBSTRING (RWSP.AccessRight,1,1)='1' THEN 'Yes' ELSE 'No' END CanRead,");
            query.Append(" CASE WHEN SUBSTRING (RWSP.AccessRight,2,1)='1' THEN 'Yes' ELSE 'No' END CanCreate,");
