...
$("#btnSave").click(function () {
    if (validateEmployee()) {
        var jsonObject = {
            ...
            "IsDualApprovalApplied": $('#Employee_IsDualApprovalApplied').is(":checked"),
...
$("#btnSavePersonalInfo").click(function () {
    if (validatePersonalInfo()) {
        var jsonObject = {
            ...
            "Photo": $('#Employee_PersonalInformation_Photo').val(),
            "Signature": $('#Employee_PersonalInformation_Signature').val(),
...
var showEmployeeData = function (data) {
    ...
    $('#Employee_IsDualApprovalApplied').prop("checked", data.IsDualApprovalApplied);
...
var showPersonalInfoData = function (data) {
    ...
    if (data.PersonalInformation.Photo != null && data.PersonalInformation.Photo.length > 0) {
        $('#photo').attr('src', uploadFilesURL + data.PersonalInformation.Photo);
        $('#Employee_PersonalInformation_Photo').val(data.PersonalInformation.Photo);
    }
    if (data.PersonalInformation.Signature != null && data.PersonalInformation.Signature.length > 0) {
        $('#signature').attr('src', uploadFilesURL + data.PersonalInformation.Signature);
        $('#Employee_PersonalInformation_Signature').val(data.PersonalInformation.Signature);
    }
...
function readURLForPhoto(input) {
    if (input.files && input.files[0]) {
        ...
        $('#Employee_PersonalInformation_Photo').val(input.files[0].name);
    }
}
...
function readURLForSignature(input) {
    if (input.files && input.files[0]) {
        ...
        reader.readAsDataURL(input.files[0]);
        $('#Employee_PersonalInformation_Signature').val(input.files[0].name);
    }
}
